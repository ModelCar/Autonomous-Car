...
vector<pair<Point,uint> > signDetection(Mat img)
{
  ...
  int x = (int)((2.0/3.0)*img.cols);
  Rect box(x, 0, img.cols-x, img.rows-20);
  work = img(box).clone();
  ...
  //tmp = blue(work, img.at<Vec3b>(Point(88,232)));//all blue signs
  //result.insert(result.end(), tmp.begin(), tmp.end());
  ...
  draw(test, red(work, img.at<Vec3b>(Point(108,232))), Scalar(0,0,255));
  draw(test, yellow(work, img.at<Vec3b>(Point(98,232))), Scalar(0,255,255));
  //draw(test, blue(work, img.at<Vec3b>(Point(88,232))), Scalar(255,0,0));
  imshow("signs", test);
  return result;
}
 ...
 */
vector<pair<Point,uint> > blue(Mat img, Vec3b color)
{
  Mat imgb;
  vector<pair<Point, uint> > signs;

  inRange(img, Scalar(80,color[1]-40,color[2]-40), Scalar(120,255,255), imgb); //filter blue
...
vector<pair<Point, uint> > findCircles(Mat img)
{
  ...
  HoughCircles(img, circles, HOUGH_GRADIENT, 2, 60, 100, 70, 15, 60);//detect the circles
...
bool equalLength(vector<Point> shape, double factor)
{
  ...
  dist /= shape.size();
  double tmp = (dist / factor) + dist;

  for(uint i = 0; i< shape.size()-1; i++)
  {
    if(sqLength(shape[i], shape[i+1]) > tmp)
      return false;
  }
  return true;
}
