...
vector<String> images = {"references/0.png","references/1.png","references/2.png",
                        "references/3.png","references/4.png","references/5.png",
                        "references/6.png","references/7.png","references/8'.png",
                        "references/vorfahrt.png"};
int redH1, redH2, redS, redV;
int yellowH1, yellowH2, yellowS, yellowV;
vector<Point > redCalibrationPoints = {Point(108, 232)};
vector<Point > yellowCalibrationPoints = {Point(98, 232)};
...
int main(int argc, char *argv[])
{
  ...
  loadReferences();//load the reference images
...
vector<pair<Point,uint> > signDetection(Mat img)
{
  ...
  int x = (int)((1.0/3.0)*img.cols);
  int y = (int)((2.0/3.0)*img.rows);
  Rect box(x, 0, img.cols-x, y);
  work = img.clone();
...
vector<pair<Point, uint> > red(Mat img)
{
  ...
  inRange(img, Scalar(redH1,redS,redV), Scalar(179,255,255), imgr1); //filter red
  inRange(img, Scalar(0,redS,redV), Scalar(redH2,255,255), imgr); //filter other red
  addWeighted(imgr1, 1.0, imgr, 1.0, 0.0, imgr);//add them

  GaussianBlur(imgr, imgr, Size(5,5), 0);//blur the image
  Canny(imgr, imgr1, 5, 50, 5);//find the edges

  imshow("filtered", imgr1);
  ...
  for(uint i = 0; i < contours.size(); i++)//loop over all contours
  {
    ...
    {
      approxPolyDP(Mat(contours[i]), approx, arcLength(Mat(contours[i]), true)*0.01, true);//approximate the contour
      if(approx.size() != 8)
        continue;
      mo = moments(approx);
      signs[k].first.x = mo.m10 / mo.m00;
...
vector<pair<Point,uint> > yellow(Mat img)
{
  ...
  inRange(img, Scalar(yellowH1,yellowS,yellowV), Scalar(yellowH2,255,255), imgy); //filter yellow
  ...
  for(uint i = 0; i < contours.size(); i++)
  {
    ...
    if(fabs(contourArea(approx)) < 40)//filter out small contours
      continue;
...
vector<pair<Point, uint> > findRedCircles(Mat img, Mat imgFiltered)
{
  ...
  HoughCircles(imgFiltered, circles, CV_HOUGH_GRADIENT, 2, 50, 100, 50, 2, 50);//detect the circles
...
int detectNumber(Mat img)
{
  ...
  for(uint i = 0; i < contours.size(); i++)
  {
    if(fabs(contourArea(contours[i])) < 50)//ignore duplicate or small contours
      continue;
...
Mat extractContours(Mat img)
{
  ...
  threshold(gray, gray, 170, 255, THRESH_OTSU);//threshold that only the black is left
...
void calibrate(Mat img){
  ...
  for (int i=0; i<redCalibrationPoints.size(); i++){
    pixelValues = img.at<uchar>(redCalibrationPoints[i]);
    H=(int) pixelValues[0];
    S=(int) pixelValues[1];
    V=(int) pixelValues[2];	
    ...
    if(H>100 && H*0.9<redH1)
      redH1=H*0.9;

    if(H<=100 && H*1.1>redH2)
      redH2=1.1*H;
  }

  for (int i=0; i<yellowCalibrationPoints.size(); i++){
    pixelValues = img.at<uchar>(yellowCalibrationPoints[i]);
    H=(int) pixelValues[0];
    S=(int) pixelValues[1];
    V=(int) pixelValues[2];	
